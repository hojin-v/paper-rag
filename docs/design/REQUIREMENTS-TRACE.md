...
| 7 | 단락별 원문/정제문/요약/키워드 | `ingest/llm_enrich.py` (Ollama JSON 강제, 폴백 포함) | **실 Ollama(1.5B)로 전 파이프라인 실행** — JSON 준수·타임아웃 폴백 동작 확인 (`docs/reports/benchmarks/…llm-cpu.md`) | ✅ (품질 합격선 판정은 운영급 HW에서 7B로 — Phase 4) |
...
| 10 | Vector DB 임베딩 저장 (단락·키워드·표) | pgvector VECTOR(1024) + HNSW 3개 + 임베딩 서버(`embed/`, hash↔BGE-M3 전환형) | **BGE-M3 실모델로 실 적재·검색** — 한국어 질의→영문 키워드 교차 언어 매칭(0.66) 실증 | ✅ |
...
| # | 요구사항 | 구현 위치 | 검증 방법 | 상태 |
| --- | --- | --- | --- | --- |
| 17 | CPU 우선·경량 모델 (Ollama Q4, BGE-M3) | `docker-compose.yml` + Settings | **compose 스택 실기동**(postgres·redis·ollama) + 마이그레이션 + CPU 추론 전 구간 실행 | ✅ (7B 품질 측정만 운영급 HW 필요) |
...
1. 실제 논문 PDF 10편 Phase 0 벤치마크 — 2단 조판·스캔본 포함, mAP/CER/TEDS/E2E F1,
   제목 추출 정확도 항목 추가 (#2)
2. 운영급 CPU 서버 또는 GPU에서 7B 품질·지연 측정 → DESIGN §6 합격선 판정 (#7 품질 게이트)
3. PP-OCR 스캔 경로 어댑터 구현 (#2)
4. 표 캡션↔표 본체 연결(table_title 매핑) 개선 (#6)
5. celery worker 병렬 배치 실증 (#19)

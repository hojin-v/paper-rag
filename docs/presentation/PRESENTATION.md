...
🔶 15편 파일럿(예비값): 레이아웃 mAP≈0.71 · OCR(정제 후) CER≈1.8% · 표 TEDS≈0.80 · 읽기순서 일치율≈0.94
— 정식 지표는 50~100편 골든셋 확대 후 재측정("측정 없이 튜닝 없다", 상세는 `EVALUATION_NOTES_AND_QA.md`)
...
| 15편 파일럿 예비 측정(mAP≈0.71·CER≈1.8%·TEDS≈0.80) | 50~100편 골든셋 정식 측정·재확인 |
| 모델·프롬프트 버전 미기록 | provenance·버전 기록 추가 |

**우선순위**: ① 실제 논문 10편 수용 테스트 ② 50~100편 평가셋 mAP/CER/TEDS/recall 정식 측정

...
class SearchService:
    ...
    def extract_keywords(self, query: str) -> list[str]:
        """LLM으로 질의에서 핵심 키워드 1~5개를 뽑아 정규화된 유니크 목록으로 반환한다.

        LLM 호출이 실패하거나 JSON 파싱에 실패하면: allow_degraded_results가
        꺼져 있으면 SearchDependencyError를 던져 검색을 중단시키고(신뢰할 수
        없는 결과를 조용히 대체하지 않기 위함), 켜져 있으면 빈 목록으로 넘어가
        아래에서 정규식 기반 폴백(_fallback_keywords)으로 대체한다. LLM이 키워드를
        하나도 못 뽑았을 때도 같은 폴백을 사용한다.
        """
    ...
    def search(self, query: str) -> SearchMatched | SearchSuggest:
        ...
        1) 질의 키워드를 추출하고 keywords/keyword_aliases와 정확 매칭을 시도한다
           (_best_exact_match). 매칭되면 바로 resolve()로 대표/연관 논문을 확정한다.
    ...
    def select(self, session_id: str, keyword_id: int) -> SearchMatched:
        ...
        SearchSessionNotFound로 취급해 404로 응답하게 한다.
        """
    ...
    def resolve(
        ...
        (4) ResultBundle 조립 (5) 엑셀 생성 및 result_id로 DB 캐시 저장, 순.
        """
...
def _fallback_keywords(query: str) -> list[str]:
    """LLM이 키워드를 하나도 주지 못했을 때 쓰는 정규식 기반 폴백: 영문 토큰과 2글자 이상 한글 토큰을 추출한다."""

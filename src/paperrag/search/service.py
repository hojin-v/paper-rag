...
그 목록의 이름 중 하나라도 포함하는 것만 결과(및 엑셀)에 담는다(`repo.paragraphs_of`의
section_query 인자, 목록 각 이름에 대소문자 무시 부분 일치, OR 결합). 논문 전체를
통짜로 돌려주는 대신 "실험 방법만", "결과만" 또는 여러 섹션 조합 같은 사용자 의도를
반영하기 위한 것이며, 어떤 논문이 대표/연관으로 선정되는지에는 영향을 주지 않는다
(선정 로직은 항상 전체 단락 기준으로 계산한다) — 이미 확정된 결과의 "무엇을
보여줄지"만 좁히는 필터다.
...
class SearchService:
    ...
    def search(
        ...
        *,
        ...
        section_query가 주어지면 최종 결과(및 엑셀)의 단락을 그 목록 중 하나라도
        포함하는 섹션명만으로 좁힌다. include_related=False면 연관 논문 조회 자체를 건너뛰고
    ...
    def resolve(
        ...
        **캐시 우선 경로**: section_query가 없고 include_related/include_tables가
        ...
        is_default_view = (
            not section_query and include_related and include_tables and include_abstract
        )
    ...
    def _bundle(
        ...
        *,
        ...
        section_query가 주어지면 단락 목록을 그 목록 중 하나라도 부분 일치(대소문자
        무시)하는 section_name으로 좁혀서 가져온다(repo.paragraphs_of). 논문 키워드 목록은 필터 대상이 아니다 —

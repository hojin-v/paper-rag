...
`primary_section_query`/`related_section_query`를 지정하면 각각 대표/연관 논문의
단락 중 `paragraphs.section_name`이 그 목록의 이름 중 하나라도 포함하는 것만
결과(및 엑셀)에 담는다(`repo.paragraphs_of`의 section_query 인자, 목록 각 이름에
대소문자 무시 부분 일치, OR 결합). 대표/연관 논문은 서로 다른 논문이라 섹션 제목
구성이 다르므로(numbering·제목이 전혀 다를 수 있음) 하나로 합친 필터를 공유하지
않고 처음부터 독립적으로 필터링한다 — 안 그러면 한쪽 논문에만 있는 섹션을 골랐을 때
다른 쪽 단락이 조용히 0건이 되는데, 어느 논문 것인지 표시 없이 하나로 합쳐 보여주면
사용자가 그 이유를 알기 어렵다. 논문 전체를 통짜로 돌려주는 대신 "실험 방법만",
"결과만" 또는 여러 섹션 조합 같은 사용자 의도를 반영하기 위한 것이며, 어떤 논문이
대표/연관으로 선정되는지에는 영향을 주지 않는다(선정 로직은 항상 전체 단락 기준으로
계산한다) — 이미 확정된 결과의 "무엇을 보여줄지"만 좁히는 필터다.

두 section_query는 자유 텍스트가 아니라 `SearchMatched.primary_available_sections`/
`related_available_sections`(각 논문에 실제 존재하는 section_name을 문서 등장
순서로 나열한 목록, `repo.available_sections`로 조회)에서 고른 값을 그대로 넣는
것을 UI 쪽 사용 방식으로 가정한다 — 사용자가 논문 구조를 미리 몰라도 실제 섹션
제목 중에서, 그리고 어느 논문 것인지 명확히 구분해 고를 수 있게 하기 위함이다.
`include_abstract=False`는 이와 별개로 논문 정보 시트의 초록 원문/요약 칸만
...
class SearchService:
    ...
    def search(
        ...
        *,
        ...
        primary_section_query/related_section_query가 주어지면 최종 결과(및 엑셀)의
        대표/연관 논문 단락을 각각 그 목록 중 하나라도 포함하는 섹션명만으로 좁힌다
        (두 논문은 섹션 구성이 다른 별개 논문이라 독립적으로 필터링한다).
        include_related=False면 연관 논문 조회 자체를 건너뛰고 응답·엑셀에서 연관
        논문 관련 항목을 전부 제외한다. include_tables=False면 표 조회를 건너뛰고
        엑셀의 표 시트를 만들지 않는다. include_abstract=False면 논문 정보 시트의
        초록 칸을 비운다. 넷 다 세션에 저장돼 select() 이후에도 그대로 유지된다.
        """
    ...
    def select(self, session_id: str, keyword_id: int) -> SearchMatched:
        ...
        primary_section_query/related_section_query/include_related/include_tables/
        include_abstract(있었다면 search() 호출 시점의 옵션)도 그대로 이어받는다.
        """
        ...
        return self.resolve(
            ...
            primary_section_query=session.primary_section_query,
            related_section_query=session.related_section_query,
    ...
    def resolve(
        ...
        **캐시 우선 경로**: primary_section_query/related_section_query가 모두 없고
        include_related/include_tables가 모두 True인 "기본 뷰" 요청이면, 먼저
        keyword_result_cache에서 이 keyword_id의 사전 계산된 결과를 찾는다. 있으면
        임베딩·점수 계산·엑셀 생성을 전부 건너뛰고 캐시된 PaperSummary와 엑셀 경로를
        그대로 재사용한다(설명 문장만 이번 질의 기준으로 새로 조립). 캐시가 없거나
        커스텀 옵션이 있으면 아래 순서로 새로 계산한다: (1) 매칭 키워드 벡터 임베딩
        (2) 대표 논문 선정(_select_primary) (3) include_related=True면
        paper_relations에서 연관 논문 조회(top_relation, 실시간 계산 없음) — False면
        이 조회 자체를 건너뛰어 related_paper가 항상 None인 응답을 만든다 (4)
        ResultBundle 조립(primary_section_query/related_section_query로 각 논문의
        단락 범위를 독립적으로 축소, include_tables=False면 표 조회도 건너뜀) (5)
        엑셀 생성 및 result_id로 DB 캐시 저장. 기본 뷰인데 캐시가 없었던 경우, 이번에
        계산한 결과를 다음 검색을 위해 keyword_result_cache에도 저장한다(지연
        워밍 — 수집 파이프라인의 사전 계산을 놓친 키워드도 첫 실제 검색에서 캐시가
        채워지게 하기 위함). 두 section_query와 include_related/include_tables 모두
        대표 논문 선정 자체에는 영향을 주지 않는다 — 이미 확정된 결과에서 "무엇을
        보여줄지"만 좁힌다. force_refresh=True면 기본 뷰라도 캐시 조회를 건너뛰고
        강제로 새로 계산한다(precompute_keyword_cache가 새 논문 적재 직후 캐시를
        무조건 갱신할 때 사용).
        """
        ...
        is_default_view = (
            not primary_section_query
            and not related_section_query
            and include_related
            and include_tables
            and include_abstract
        )
        if is_default_view and not force_refresh:
            ...
            if cached is not None:
                return SearchMatched(
                    ...
                    primary_available_sections=self.repo.available_sections(
                        cached.primary_paper.paper_id
                    ),
                    related_available_sections=(
                        self.repo.available_sections(cached.related_paper.paper_id)
                        if cached.related_paper
                        else []
                    ),
                )
        ...
        return SearchMatched(
            ...
            primary_paper=primary_summary,
            related_paper=related_summary,
            primary_available_sections=self.repo.available_sections(primary_meta.paper_id),
            related_available_sections=(
                self.repo.available_sections(related_meta.paper_id)
                if related_meta is not None
                else []
            ),
        )
    ...
    def _bundle(
        ...
        *,
        ...
        primary_section_query/related_section_query가 주어지면 각각 대표/연관 논문의
        단락 목록을 그 목록 중 하나라도 부분 일치(대소문자 무시)하는 section_name으로
        독립적으로 좁혀서 가져온다(repo.paragraphs_of) — 두 논문은 섹션 제목 구성이
        다른 별개 논문이라 같은 필터를 공유하면 한쪽에만 있는 섹션을 고를 때 다른
        쪽 단락이 조용히 0건이 되는 문제가 있다. 논문 키워드 목록은 필터 대상이
        아니다 — 섹션 필터는 어디까지나 "이번 산출물에 어떤 단락 텍스트를 포함할지"를
        사용자가 고르는 기능이라서다. include_tables=False면 표 조회(repo.tables_of)
        자체를 건너뛰어 불필요한 DB 왕복을 없애고, 결과 ResultBundle.include_tables가
        ...
        primary_paragraphs = _paragraph_infos(
            self.repo.paragraphs_of(primary_meta.paper_id, section_query=primary_section_query)
        )
        related_paragraphs = (
            _paragraph_infos(
                self.repo.paragraphs_of(related_meta.paper_id, section_query=related_section_query)
            )
            if related_meta is not None
            else []
        )

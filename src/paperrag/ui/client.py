...
class ApiClient:
    ...
    def run_automatic_ocr_async(self, document_id: str) -> str:
        """`run_automatic_ocr`와 동일한 작업을 백그라운드 큐에 제출하고 즉시 task_id를 반환한다.

        200 DPI 논문 1페이지가 5분 이상 걸릴 수 있어(docs/guide/10) 이 요청 자체는
        길게 블로킹하지 않는다 — 실제 처리 상태는 `job_status(task_id)`로 폴링한다.
        """
        response = self._request("POST", f"/documents/{document_id}/auto-ocr/async")
        return str(response.json()["task_id"])

    def run_reviewed_ocr_async(self, document_id: str) -> str:
        """`run_document_ocr`와 동일한 작업을 백그라운드 큐에 제출하고 task_id를 반환한다."""
        response = self._request("POST", f"/documents/{document_id}/run-ocr/async")
        return str(response.json()["task_id"])

    def job_status(self, task_id: str) -> dict[str, Any]:
        """비동기 OCR 작업의 진행 상태(pending/started/success/failure)를 조회한다.

        success면 result 키에 갱신된 ReviewDocument(JSON dict)가, failure면 error
        키에 예외 메시지가 담겨 온다(review.models.JobStatus와 같은 모양).
        """
        response = self._request("GET", f"/jobs/{task_id}")
